...
- [ ] **Save** in Chrome/Edge: the first Save asks for a file and writes it, and later Saves write to the same file with no prompt. Open `GameEngine/levels/level_05.json` with **Open file…**, edit, Save, and `git diff` shows only your change. In a browser without the File System Access API, Save downloads `level_XX.json`.
