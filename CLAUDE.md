...
- **Slope limits:** the hitbox rests on its higher corner on a slope (the sprite is drawn lower to compensate). Only 45° slopes and quarter circles exist: no half-height (26°/63°) slopes yet, and no arbitrary polygons (add pieces to `tileShapes.js` when needed). A ceiling contact blocks a grounded player moving under it rather than sliding along it. Frame-time spikes can tunnel through geometry, same as full blocks; fixed-timestep physics is on the roadmap.
...
3. ~~**Sloped and curved geometry**~~ ✓ Done — 45° slopes and quarter circles (convex and concave) in four orientations each, SAT collision, tile ids `10-33` (see `tileShapes.js`). Half-height slopes and arbitrary polygons are future additions.
